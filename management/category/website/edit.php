<!--Sửa danh mục cha-->
<div class="container">
<div class="modal fade" id="editCha" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog " role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Sửa danh mục website</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <div class="card-body card-block">
                    <div class="form-group ">
                        <form >
                            <label for="tên danh mục" style=" margin-right: 20px;">Tên danh mục </label>
                            <input type="text" id="category_name" class="form-control" placeholder="Nhập vào tên danh mục">
                        </form>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="border rounded border-secondary btn btn-secondary" data-dismiss="modal">Đóng</button>
                <button type="button" class="border rounded border-success btn btn-success">Sửa</button>
            </div>
        </div>
    </div>
</div>
</div>
<!--Sửa danh mục cha-->

<!--Sửa danh mục Con-->
<div class="container">
    <div class="modal fade" id="editCon" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog " role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Sửa danh mục website</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="card-body card-block">
                        <div class="form-group ">
                            <form >
                                <label for="tên danh mục" style=" margin-right: 20px;">Tên danh mục </label>
                                <input type="text" id="category_name" class="form-control" placeholder="Nhập vào tên danh mục">

                                <label for="danh mục cha">Danh mục cha </label>
                                <select type="text" id="position_name" class="form-control" >
                                    <option>1</option>
                                    <option>2</option>
                                    <option>3</option>
                                    <option>4</option>
                                </select>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="border rounded border-secondary btn btn-secondary" data-dismiss="modal">Đóng</button>
                    <button type="button" class="border rounded border-success btn btn-success">Sửa</button>
                </div>
            </div>
        </div>
    </div>
</div>
<!--Sửa danh mục Con-->